...
export function boldRule(markType: MarkType) {
  return new InputRule(/\*\*([^*]+)\*\*$/, (state: EditorState, match: RegExpMatchArray, start: number, end: number): Transaction | null => {
    const text = match[1]
...
export function boldRuleUnderscore(markType: MarkType) {
  return new InputRule(/__([^_]+)__$/, (state: EditorState, match: RegExpMatchArray, start: number, end: number): Transaction | null => {
    const text = match[1]
    const mark = markType.create()
    return state.tr.replaceWith(start, end, state.schema.text(text, [mark]))
  })
...
export function italicRuleUnderscore(markType: MarkType) {
  return new InputRule(/_([^_]+)_$/, (state: EditorState, match: RegExpMatchArray, start: number, end: number): Transaction | null => {
    const text = match[1]
    const mark = markType.create()
    return state.tr.replaceWith(start, end, state.schema.text(text, [mark]))
  })
...
export function underlineRule(markType: MarkType) {
  return new InputRule(/<u>(.*?)<\/u>$/, (state: EditorState, match: RegExpMatchArray, start: number, end: number): Transaction | null => {
    const text = match[1]
    const mark = markType.create()
    return state.tr.replaceWith(start, end, state.schema.text(text, [mark]))
  })
...
export function buildInputRules(schema: Schema) {
  ...
  if (type = schema.marks.underline) rules.push(underlineRule(type as MarkType))
  if (type = schema.marks.strike) rules.push(strikethroughRule(type as MarkType))
